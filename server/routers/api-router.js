...
router.post("/api/add-monitor", async (request, response) => {
    ...
    monitor.dns_resolve_type = null;
    monitor.dns_resolve_server = null;
    monitor.dns_last_result = null;
    monitor.docker_container = null;
    ...
    monitor.rabbitmqUsername = null;
    monitor.rabbitmqPassword = null;
        ...
        try {
            ...
            if (!publicMonitor) {
                ...
                badgeValues.label = filterAndJoin([
                    labelPrefix,
                    label ??
                        `Uptime (${requestedDuration.slice(
                            0,
                            -1
                        )}${labelSuffix})`,
                ]);
...
router.get(
    ...
    async (request, response) => {
        ...
        try {
            ...
            const uptimeCalculator = await UptimeCalculator.getUptimeCalculator(
                requestedMonitorId
            );
            const publicAvgPing =
            ...
            if (!publicAvgPing) {
                ...
                badgeValues.label = filterAndJoin([
                    labelPrefix,
                    label ??
                        `Avg. Ping (${requestedDuration.slice(
                            0,
                            -1
                        )}${labelSuffix})`,
                ]);
